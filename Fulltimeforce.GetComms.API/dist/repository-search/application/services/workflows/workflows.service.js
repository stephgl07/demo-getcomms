...
let WorkflowsService = class WorkflowsService {
    ...
    async getWorkflows(repoName, page, per_page) {
        ...
        const response = await this.githubRequestHandler.Get(`${baseUrl}/repos/${user}/${repoName}/actions/workflows`, {
            headers,
            params
        });
        return response.data;
    }
    async getWorkflowsRuns(repoName, workflowId, page, per_page) {
        const baseUrl = this.configService.get('API_BASE_URL');
        const user = this.configService.get('API_USER');
        const token = this.configService.get('API_TOKEN');
        const headers = {};
        ...
        const response = await this.githubRequestHandler.Get(`${baseUrl}/repos/${user}/${repoName}/actions/workflows/${workflowId}/runs`, {
            headers,
            params
        });
        return response.data;
